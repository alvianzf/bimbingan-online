...
if($role == 'mahasiswa')
{
	$ss = mysql_fetch_assoc(mysql_query("SELECT * FROM tb_bimbingan WHERE nim_id= $id AND jenis_bimbingan = 'Kuliah Praktek'"));
	$mhs = $ss['nim_id'];
	$dos = $ss['nidn_id'];
	...
	$ss = mysql_fetch_assoc(mysql_query("SELECT * FROM tb_bimbingan WHERE nidn_id= $id AND jenis_bimbingan = 'Kuliah Praktek'"));
	$mhs = $ss['nim_id'];
	$dos = $ss['nidn_id'];
	$na = mysql_fetch_assoc(mysql_query("SELECT * FROM tb_mahasiswa WHERE nim_id = $mhs "));
	$ma = mysql_fetch_assoc(mysql_query("SELECT * FROM tb_dosen WHERE nidn = $dos "));
...
<div style="text-align: center;">
	<h3 >Laporan Bimbingan Kerja Praktek Mahasiswa</h3>
	<h4><?=$nama;?></h4>
</div>
<br/><br/>
...
<div style="align-items: center;">
	<table style="border: 1px solid black; border-collapse: collapse;">
		...
		<tr>
			...
			<th style="border: 1px solid black; padding: 5px">Daftar Perubahan</th>
			<th style="border: 1px solid black; padding: 5px">Revisi</th>
		</tr>
		</thead>
		<tbody>
...
<?php
		...
		if($role =='mahasiswa')
		{
		$sql= mysql_query("SELECT * FROM tb_bimbingan WHERE nim_id= $mhs AND jenis_bimbingan = 'Kuliah Praktek'");
		}else{
		$sql= mysql_query("SELECT * FROM tb_bimbingan WHERE nidn_id = $dos AND jenis_bimbingan = 'Kuliah Praktek'");
		}
